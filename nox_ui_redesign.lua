...
local function ResetPlayer()
    ...
    resetThread = task.spawn(function()
        ...
        while character and character.Parent and humanoid and humanoid.Health > 0
            and not isRespawning and not stopResetSequence and myLockId == resetLockId do
...
do
    ...
    local function tryReset()
        if cooldown or resetCooldown or thisScriptStopped then return end
        cooldown = true
        pcall(ResetPlayer)
        task.delay(1.2, function() cooldown = false end)
    end

    local function isBallooned(char)
        if not char then return false end
        if char:FindFirstChild("Balloon") or char:FindFirstChild("BalloonConstraint") then return true end
        for _, d in ipairs(char:GetDescendants()) do
            local n = string.lower(d.Name)
            if n:find("balloon") then return true end
        end
        if LocalPlayer:GetAttribute("jump higher") or LocalPlayer:GetAttribute("JumpHigher") then return true end
        if char:GetAttribute("jump higher") or char:GetAttribute("JumpHigher") then return true end
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then
            if hum:GetAttribute("jump higher") or hum:GetAttribute("JumpHigher") then return true end
            if hum.JumpHeight and hum.JumpHeight > 15 then return true end
        end
        return false
    end
    ...
    local function isTiny(char)
        if not char then return false end
        if char:FindFirstChild("Tiny") then return true end
        local scale = char:GetAttribute("Scale") or char:GetAttribute("Tiny") or char:GetAttribute("IsTiny")
        if type(scale) == "boolean" and scale then return true end
        if type(scale) == "number" and scale < 0.7 then return true end
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then
            local hs = hum:FindFirstChild("BodyHeightScale")
            if hs and hs:IsA("NumberValue") and hs.Value < 0.7 then return true end
            if hum:GetAttribute("Tiny") or hum:GetAttribute("IsTiny") then return true end
        end
        for _, d in ipairs(char:GetChildren()) do
            if string.lower(d.Name):find("tiny") then return true end
        end
        return false
    end
    ...
    local function isJailed(char)
        if not char then return false end
        if LocalPlayer:GetAttribute("Jailed") or LocalPlayer:GetAttribute("Jail") or LocalPlayer:GetAttribute("InJail") then return true end
        if char:GetAttribute("Jailed") or char:GetAttribute("Jail") or char:GetAttribute("InJail") then return true end
        if char:FindFirstChild("Jail") or char:FindFirstChild("Jailed") then return true end
        for _, d in ipairs(char:GetChildren()) do
            if string.lower(d.Name):find("jail") then return true end
        end
        return false
    end
    ...
    local function hook(char)
        ...
        table.insert(ActiveConnections, char.DescendantAdded:Connect(function(obj)
            if thisScriptStopped then return end
            local n = string.lower(obj.Name or "")
            if _G.AutoResetOnBalloon and n:find("balloon") then tryReset()
            elseif _G.AutoResetOnTiny and n:find("tiny") then tryReset()
            elseif _G.AutoResetOnJail and n:find("jail") then tryReset()
            end
        end))
        local function watch(inst, attr, flag)
            pcall(function()
                table.insert(ActiveConnections, inst:GetAttributeChangedSignal(attr):Connect(function()
                    if thisScriptStopped or not _G[flag] then return end
                    if inst:GetAttribute(attr) then tryReset() end
                end))
            end)
        end
    ...
    task.spawn(function()
        while not thisScriptStopped do
            ...
            local char = LocalPlayer.Character
            if char then
                if _G.AutoResetOnBalloon and isBallooned(char) then tryReset()
                elseif _G.AutoResetOnTiny and isTiny(char) then tryReset()
                elseif _G.AutoResetOnJail and isJailed(char) then tryReset()
                end
            end
        end
    end)

    table.insert(ActiveConnections, LocalPlayer.CharacterAdded:Connect(function(c)
        task.defer(function()
            task.wait(0.15)
            hook(c)
        end)
    ...
    if LocalPlayer.Character then
        task.defer(function() hook(LocalPlayer.Character) end)
    end
end
...
if _G.QuickPickup then QuickPickup.set(true) end
...
local function saveKeybinds()
    pcall(function()
        writefile(KB_FILE, HttpService:JSONEncode({
...
local function fireCommandOnPlayer(cmdName, targetPlayer)
    ...
    if not profileButton or not cmdButton then return false end
...
local SlotsConfig = {
    ...
    [10] = { Positions = { Vector3.new(-348.2407,-7.5033,74.3719), Vector3.new(-332.8,-6.6,67.8) }, CamOffset = Vector3.new(-377.7117,8.9106,25.7208)-Vector3.new(-332.8,-6.6,67.8), CamAngles = {-0.3277,-0.0150,0} },
...
local function clearSlotMarker()
    if currentSlotMarker then
        pcall(function() currentSlotMarker:Destroy() end)
...
local function findBrainrotModel(prompt)
    ...
    if basePart then
        if basePart:IsA("BasePart") then refPos = basePart.Position
        elseif basePart:IsA("Model") then
            local bp = basePart.PrimaryPart or basePart:FindFirstChildWhichIsA("BasePart")
            if bp then refPos = bp.Position end
        end
    end

    local function getModelPos(v)
        local rp = v.PrimaryPart or v:FindFirstChild("RootPart") or v:FindFirstChild("FakeRootPart") or v:FindFirstChild("HumanoidRootPart") or v:FindFirstChildWhichIsA("BasePart")
    ...
    for _, v in ipairs(plotsFolder:GetDescendants()) do
        if v:IsA("Model") and not playerNames[v.Name] and v.Name ~= "Base" and v.Name ~= "Podium" then
            local pos = getModelPos(v)
            if pos then
                local d = (pos - refPos).Magnitude
                if d < 15 then
                    local score = d
                    if petName and petName ~= "" and v.Name == petName then score = d - 0.5 end
                    if score < bestScore then bestScore = score bestModel = v end
                end
            end
        end
    end
...
local function createSlotMarker(prompt)
    clearSlotMarker()
    if not prompt or not _G.SlotESP then return end
...
local function getSelectedCarpetTool()
    ...
    local searchOrder = {}
    if choice == "broom" then searchOrder = {"broom", "witch"}
    elseif choice == "sleigh" then searchOrder = {"sleigh", "santa"}
    elseif choice == "wings" then searchOrder = {"wings", "cupid"}
    elseif choice == "waverider" then searchOrder = {"waverider", "wave rider", "wave"}
    elseif choice == "auto" then searchOrder = {"waverider", "wave rider", "wings", "cupid", "sleigh", "santa", "broom", "witch", "carpet", "magic carpet", "flying carpet", "rainbow"}
    else searchOrder = {"flying carpet", "magic carpet", "rainbow carpet", "carpet"} end
    ...
    returnMovement = RunService.Heartbeat:Connect(function()
        ...
        if not waypointReached then
            ...
            if wpDist <= BASE_STOP_DIST then
                ...
                Velocity.VectorVelocity = Vector3.new(moveDir.X * BASE_SPEED * speedMult, vertVel, moveDir.Z * BASE_SPEED * speedMult)
                return
            end
        end
...
local function isInFlashSafeZone(pos)
    return pos.X >= FLASH_SAFE_ZONE.minX and pos.X <= FLASH_SAFE_ZONE.maxX
       and pos.Z >= FLASH_SAFE_ZONE.minZ and pos.Z <= FLASH_SAFE_ZONE.maxZ
end
    ...
    currentMovement = RunService.Heartbeat:Connect(function()
        ...
        if dist <= STOP_DIST then
            ...
            elseif config.CamOffset then
                ...
                Camera.CFrame = CFrame.new(Root.Position + Vector3.new(0, 5, 0)) * CFrame.Angles(unpack(config.CamAngles))
            end
            ...
            if flash then
                ...
                if slotNumber == 6 then
                    Camera.CameraType = Enum.CameraType.Scriptable
                    Camera.CFrame = CFrame.new(-301.68, -10.02, 39.60, 1.000, -0.020, -0.020, 0.000, 0.714, -0.700, 0.028, 0.700, 0.714)
                end
            end
...
table.insert(ActiveConnections, LocalPlayer.CharacterAdded:Connect(function()
    if not _G.AutoFlashTP then return end
    if not selectedPrompt or not selectedSlotNumber then return end
...
updatePetList = function()
    ...
    for _, petData in ipairs(tempPets) do
        ...
        task.spawn(function()
            ...
            if not plotsFolder2 then return end
            local playerNames = {}
            for _, p in pairs(Players:GetPlayers()) do playerNames[p.Name] = true end
            ...
            for _, v in ipairs(plotsFolder2:GetDescendants()) do
            ...
            if not foundModel and petData.spawn then
                ...
                for _, v in ipairs(plotsFolder2:GetDescendants()) do
                    if v:IsA("Model") and not playerNames[v.Name] and v.Name ~= "Base" and v.Name ~= "Podium" then
                        local rp = v.PrimaryPart or v:FindFirstChild("RootPart") or v:FindFirstChild("FakeRootPart") or v:FindFirstChildWhichIsA("BasePart")
                        if rp then
    ...
    if #tempPets == 0 then
        ...
        local es = Instance.new("UIStroke"); es.Color = Color3.fromRGB(45, 45, 45); es.Parent = emptyCard
...
ScrollingFrame2.Parent = SettingsContent
...
local function toggleRow(title, desc, defaultOn, onToggle)
    ...
    Instance.new("UICorner", row).CornerRadius = UDim.new(0, 8)
    local s = Instance.new("UIStroke"); s.Color = C.strokeDim; s.Parent = row
    ...
    t2.Size = UDim2.new(1, -52, 0, 18)
    t2.Position = UDim2.new(0, 12, 0, 20)
    ...
    local function render(animate)
        ...
        if ref.on then
            ...
            TweenService:Create(knob, info, { Position = UDim2.new(0, 4, 0.5, -6), BackgroundColor3 = C.knobOff }):Play()
        end
    end
    render(false)
...
toggleRow("Reset On Balloon", "Auto reset when ballooned", _G.AutoResetOnBalloon, function(v) _G.AutoResetOnBalloon = v end)
toggleRow("Reset On Tiny", "Auto reset when tiny", _G.AutoResetOnTiny, function(v) _G.AutoResetOnTiny = v end)
toggleRow("Reset On Jail", "Auto reset when jailed", _G.AutoResetOnJail, function(v) _G.AutoResetOnJail = v end)

sectionHeader("  FLASH TP  ")
toggleRow("Auto Block", "Block all players after Flash", _G.AutoBlock, function(v) _G.AutoBlock = v end)
toggleRow("Auto Giant", "Giant potion after Flash", _G.AutoGiant, function(v) _G.AutoGiant = v end)
toggleRow("Bypass Ragdoll", "Ragdoll after Flash", _G.RagdollBypass, function(v) _G.RagdollBypass = v end)
toggleRow("Anti Swap", "Tiny+Rocket on closest", _G.AntiSwap, function(v) _G.AntiSwap = v end)
toggleRow("Auto Flash TP", "Auto Flash after reset", _G.AutoFlashTP, function(v) _G.AutoFlashTP = v end)
...
do
    ...
    local lagStroke = Instance.new("UIStroke"); lagStroke.Color = C.strokeDim; lagStroke.Parent = lagRow
    ...
    table.insert(ActiveConnections, UserInputService.InputChanged:Connect(function(inp)
        if draggingLag and (inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch) then
            local pct = (inp.Position.X - lagTrack.AbsolutePosition.X) / lagTrack.AbsoluteSize.X
            updateLaggerPower(pct)
        end
    end))
...
toggleRow("Auto Balloon", "Balloon brainrots in zones", _G.AutoBalloon, function(v) _G.AutoBalloon = v if not v then BalloonState.Active = {} end end)
...
toggleRow("Slot ESP", "Red highlight on brainrot", _G.SlotESP, function(v)
    _G.SlotESP = v
    if v and selectedPrompt then createSlotMarker(selectedPrompt) else clearSlotMarker() end
end)
toggleRow("Auto Select Best", "Auto select lowest slot", _G.AutoSelectBest, function(v) _G.AutoSelectBest = v end)
...
local bdStroke = Instance.new("UIStroke"); bdStroke.Color = C.strokeDim; bdStroke.Parent = bdContainer
...
local function makeDelayBtn(text, key, x)
    ...
    local st = Instance.new("UIStroke"); st.Color = _G.BlockDelay == key and C.accent or C.stroke; st.Parent = btn
    btn.MouseButton1Click:Connect(function()
        _G.BlockDelay = key
        saveSettings()
        for _, child in ipairs(bdContainer:GetChildren()) do
            if child:IsA("TextButton") then
                local isActive = (child.Text == "FAST" and key == "fast") or (child.Text == "NORMAL" and key == "normal") or (child.Text == "SLOW" and key == "slow")
                ...
                if s then s.Color = isActive and C.accent or C.stroke end
            end
        end
    end)
    return btn
...
carpetContainer.Parent = ScrollingFrame2
Instance.new("UICorner", carpetContainer).CornerRadius = UDim.new(0, 8)
local carpetStroke = Instance.new("UIStroke"); carpetStroke.Color = C.strokeDim; carpetStroke.Parent = carpetContainer
        ...
        keyBtn.MouseButton1Click:Connect(function()
            ...
            conn = UserInputService.InputBegan:Connect(function(inp, gpe)
                ...
                if actionName == "flash" then
                    ...
                    if lbl then lbl.Text = "FLASH [" .. inp.KeyCode.Name .. "]" end
...
-- ==========================================
local locked = false
local function syncBorder() BorderFrame.Position = Win.Position end
...
TabBrainrots.MouseButton1Click:Connect(function() setTab("brainrots") end)
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    if minimised then
        local targetWinPos = UDim2.new(currentWinPos.X.Scale, currentWinPos.X.Offset, currentWinPos.Y.Scale, currentWinPos.Y.Offset - offset)
        ...
        TweenService:Create(BorderFrame, info, { Size = UDim2.new(0, L.winW+4, 0, MIN_BORDER_H), Position = targetWinPos }):Play()
    else
        local targetWinPos = UDim2.new(currentWinPos.X.Scale, currentWinPos.X.Offset, currentWinPos.Y.Scale, currentWinPos.Y.Offset + offset)
...
local function hookButton(btn, normal, hover)
    btn.MouseEnter:Connect(function() TweenService:Create(btn, TweenInfo.new(0.12), { BackgroundColor3 = hover }):Play() end)
    btn.MouseLeave:Connect(function() TweenService:Create(btn, TweenInfo.new(0.12), { BackgroundColor3 = normal }):Play() end)
    btn.MouseButton1Down:Connect(function() TweenService:Create(btn, TweenInfo.new(0.06), { BackgroundColor3 = C.deepBlue }):Play() end)
    btn.MouseButton1Up:Connect(function() TweenService:Create(btn, TweenInfo.new(0.1), { BackgroundColor3 = hover }):Play() end)
end
...
FLASHTP.MouseButton1Click:Connect(function()
    if selectedPrompt and selectedSlotNumber then
        if not isStealing and not autoStealEnabled then
            ...
            if _G.AntiSwap then
                task.spawn(function() pcall(executeTinyAndRocket) end)
            end
            startTripToPetSlot(selectedPrompt, selectedSlotNumber)
        end
    end
...
task.defer(function()
    local lbl = FLASHTP:FindFirstChildOfClass("TextLabel")
    if lbl then lbl.Text = "FLASH [" .. keyNames.flash .. "]" end
...
local kbConn = UserInputService.InputBegan:Connect(function(inp, gpe)
    ...
    if inp.KeyCode == keybinds.flash then
        if selectedPrompt and selectedSlotNumber and not isStealing and not autoStealEnabled then
            if _G.AntiSwap then task.spawn(function() pcall(executeTinyAndRocket) end) end
            startTripToPetSlot(selectedPrompt, selectedSlotNumber)
        end
...
pcall(function() clearTpPathVisual() end)
pcall(function() clearSlotMarker() end)
